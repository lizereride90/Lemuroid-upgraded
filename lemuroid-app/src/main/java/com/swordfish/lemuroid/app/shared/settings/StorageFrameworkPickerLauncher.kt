...
class StorageFrameworkPickerLauncher : RetrogradeActivity() {
    ...
    private fun createSystemFolders(treeUri: Uri) {
        val childrenUri = DocumentsContract.buildDocumentUriUsingTree(
            treeUri,
            DocumentsContract.getTreeDocumentId(treeUri),
        )
        SYSTEM_FOLDERS.forEach { name ->
            try {
                DocumentsContract.createDocument(contentResolver, childrenUri, DocumentsContract.Document.MIME_TYPE_DIR, name)
            } catch (_: Exception) {
            }
        }
    }
